...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["action"])) {
    ...
    if ($_POST["action"] == "add_task") {
        ...
        if (!empty($title)) {
            ...
            $stmt = $conn->prepare("INSERT INTO tasks (user_id, title, status, priority, due_date) VALUES (?, ?, 'pending', ?, ?)");
...
// Load tasks — pending first, then completed; include due_date

$stmt = $conn->prepare("SELECT id, title, status, priority, due_date, created_at FROM tasks WHERE user_id=? ORDER BY FIELD(status,'pending','completed'), FIELD(priority,'high','medium','low'), created_at DESC");
...
<main class="page">
  ...
  <div class="panel">
    ...
    <input
      ...
      style="margin-bottom:14px; width:100%; padding:10px 12px; border:1px solid #ddd; border-radius:6px; font-size:14px;">
